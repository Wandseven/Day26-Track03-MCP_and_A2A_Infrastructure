    ...
    messages = [
        SystemMessage(
            content=(
                ...
                'needs_privacy = true → question involves data privacy, GDPR, CCPA, user data'
            )
        ),
        HumanMessage(content=state["question"]),
    ]
...
async def call_privacy_specialist(state: LegalState) -> dict:
    ...
    privacy_prompt = (
        "You are an expert in GDPR, CCPA, and global data privacy laws. "
        "Analyse the privacy implications of the question. Keep your response under 200 words."
    )
...
async def main():
    ...
    # --- Step 3: Draw graph (Added for Exercise) ---
    try:
        graph_png = graph.get_graph().draw_mermaid_png()
        with open("graph_stage4.png", "wb") as f:
            f.write(graph_png)
        print("\n[Step 3] Graph visualization saved to 'graph_stage4.png'")
    except Exception as e:
        print(f"\n[Step 3] Could not generate graph image: {e}")
        print("Note: Graph visualization requires 'pygraphviz' or 'mermaid' dependencies.")
